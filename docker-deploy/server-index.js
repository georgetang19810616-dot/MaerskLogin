...
// 发送截图到客户端
function sendScreenshot(taskId, screenshotBase64) {
  const ws = clients.get(taskId);
  if (ws && ws.readyState === WebSocket.OPEN) {
    ws.send(JSON.stringify({
      type: 'screenshot',
...
app.post('/api/tasks', async (req, res) => {
  try {
    ...
    // 异步执行任务
    const automation = new MaerskAutomation(
      config,
      (level, message) => sendLog(taskId, level, message),
      (result) => sendResult(taskId, result),
      (screenshot) => sendScreenshot(taskId, screenshot)
    );
...
app.get('/api/vnc/:taskId', (req, res) => {
  ...
  res.json({
    success: true,
    vnc: {
      connected: task ? task.status === 'running' : false,
      host: process.env.VNC_HOST || 'localhost',
      port: process.env.VNC_PORT || 5900,
      url: `http://${req.headers.host}:6080/vnc.html`,
    },
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(`VNC地址: http://localhost:6080/vnc.html`);
  console.log(`=================================`);
});
